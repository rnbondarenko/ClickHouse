...
"""
    ...
    def create_cherrypick(self):
        ...
        for branch in [self.cherrypick_branch, self.backport_branch]:
            git_runner(f"{self.git_prefix} push -f origin {branch}:{branch}")
    ...
    def create_backport(self):
        ...
        git_runner(f"{self.git_prefix} pull --ff-only origin {self.backport_branch}")
        merge_base = git_runner(
            f"{self.git_prefix} merge-base origin/{self.name} {self.backport_branch}"
        )
        ...
        git_runner(
            f"{self.git_prefix} push -f origin "
...
class Backport:
    ...
    def receive_prs_for_backport(self):
        since_commit = git_runner(
            f"git merge-base {self.release_branches[0]} {self.default_branch}"
        )
